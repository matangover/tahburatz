...
route_ids = [
#### 14:
'10179', # other direction
'10180',   # Weekdays
'12834', # other direction
'12835', # other direction
'12836',   # Friday
'12837',    # Saturday
...
def construct_request(stop_ids):
    ...
    monitoring_requests = "\n".join(
                """<siri:StopMonitoringRequest version="IL2.7" xsi:type="siri:StopMonitoringRequestStructure">
                    ...
                    <siri:LineRef>{line_id}</siri:LineRef>
...
def construct_request_by_lines(line_ids):
    timestamp = get_timestamp()
    monitoring_requests = "\n".join(
                """<siri:StopMonitoringRequest version="IL2.7" xsi:type="siri:StopMonitoringRequestStructure">
                    <siri:RequestTimestamp>{timestamp}</siri:RequestTimestamp>
                    <siri:MessageIdentifier xsi:type="siri:MessageQualifierStructure">{message_id}</siri:MessageIdentifier>
                    <siri:PreviewInterval>PT10M</siri:PreviewInterval>
                    <siri:MonitoringRef xsi:type="siri:MonitoringRefStructure">all</siri:MonitoringRef>
                    <siri:LineRef>{line_id}</siri:LineRef>
                    <siri:MaximumStopVisits>100</siri:MaximumStopVisits>
                </siri:StopMonitoringRequest>""".format(timestamp=timestamp, line_id=line_id, message_id=i)
...
def send_request():
    request_data = construct_request_by_lines(route_ids)
